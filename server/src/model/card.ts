...
function isSpecialPattern(pattern: Pattern) : boolean {
    if (pattern === 'dog' || pattern === 'phoenix' || pattern === 'mahjong' || pattern === 'dragon') {
        return true;
    }
    return false;
}
...
export class Single extends CardCombination {
    constructor({cards, start = false}: {cards: Array<Card>, start?: boolean}) {
        super();
    ...
    lessThan(other: Single): boolean {
        ...
        } else if (this.cards[0].hasNumber() && other.cards[0].pattern === 'dragon') {
            return true;
        } else if (this.cards[0].pattern === 'dragon' && other.cards[0].hasNumber()) {
            return false;
        } else {
            throw new Error('incomparable cards');
        }
...
export class Triplet extends CardCombination {
    ...
    static isSuited(cards: Array<Card>): boolean {
        ...
        if (!cards[0].hasNumber() || !cards[1].hasNumber() || !cards[2].hasNumber()) return false;
...
export function findCombination(cards: Array<Card>): CardCombination {
    for (let item of vaildCombination) {
        if (item[1].isSuited(cards)) {
            return new item[1]({cards});
        }
    }
